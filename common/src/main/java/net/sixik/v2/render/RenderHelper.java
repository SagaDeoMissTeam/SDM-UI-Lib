    ...
    public static void addRectToBufferWithUV(GuiGraphics graphics, BufferBuilder buffer, int x, int y, int w, int h, RGB rgb, float u0, float v0, float u1, float v1) {
        if (w > 0 && h > 0) {
            ...
            if(rgb instanceof RGBA rgba)
                a = rgba.a;
            buffer.vertex(m, (float)x, (float)(y + h), 0.0F).color(r, g, b, a).uv(u0, v1).endVertex();
            buffer.vertex(m, (float)(x + w), (float)(y + h), 0.0F).color(r, g, b, a).uv(u1, v1).endVertex();
            buffer.vertex(m, (float)(x + w), (float)y, 0.0F).color(r, g, b, a).uv(u1, v0).endVertex();
            buffer.vertex(m, (float)x, (float)y, 0.0F).color(r, g, b, a).uv(u0, v0).endVertex();
        }
    }
